...
/* Date: 07-22-2015                                                   */
/* Pair wise fit alignment without affine gap.                        */
/* Really depends on which one fits to which.                         */
/* Let's say s1 fits to s2.                                           */
/* initilize S(i,j):                                                  */
/* S(i, 0) = -INF and S(0, j) = 0                                     */
...
/* tracing back from there and stop when we get to the top row        */
/*--------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
matrix_t *create_matrix(size_t m, size_t n){
	...
	for(i=0; i < S->m; i++) S->score[i][0] = -INFINITY;
